...
internal class RateLimiter(
    ...
    suspend fun <T> execute(block: suspend () -> T): T {
        ...
        for (attempt in 0 until maxRetries) {
            try {
                ...
                if (e.code == 99991400) {
                    logger.warn {
                        "Rate limit error encountered (99991400), retry attempt ${attempt + 1}/$maxRetries"
                    }
                    lastException = e
    ...
    private suspend fun waitForAvailableSlot() {
        mutex.withLock {
            ...
            if (requestTimes.size >= maxRequestsPerSecond) {
                ...
                if (waitTime > 0) {
                    logger.debug {
                        "Rate limit reached (${requestTimes.size}/$maxRequestsPerSecond), waiting ${waitTime}ms"
                    }
                    delay(waitTime)

                    // Clean old records again
                    val newNow = Clock.System.now().toEpochMilliseconds()
                    requestTimes.removeAll { it < newNow - 1000 }
                }
            }
        }
    }
    ...
    private suspend fun recordRequest() {
        mutex.withLock {
            ...
            logger.trace {
                "Request recorded, current window requests: ${requestTimes.size}/$maxRequestsPerSecond"
            }
        }
    }
    ...
    private fun calculateBackoff(attempt: Int): Long {
        ...
        val jitter = (kotlin.random.Random.nextDouble() * 500).toLong() // 0-500ms random jitter
